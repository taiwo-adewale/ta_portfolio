  ...
  return (
    <>
      ...
      <div className="flex-center flex-row flex-wrap mt-16 mb-8">
        {["Tailwind", "React JS", "Next JS", "All"].map((item, index) => (
          <div
            key={index}
            onClick={() => handleWorkFilter(item)}
            className={`py-2 px-4 rounded-lg bg-white font-extrabold cursor-pointer transition-all duration-300 ease-in m-2 text-black flex-center p-text hover:bg-secondary hover:text-white xxl:py-4 xxl:px-8 xxl:rounded-[0.85rem] ${
      ...
      >
        {filterWork.map((work, index) => (
          <div
            className="xs:w-full xs:mx-0 w-[270px] flex-col m-8 p-4 rounded-lg flex-center bg-white text-black cursor-pointer transition-all duration-300 ease-in hover:shadow-[0_0_25px_rgba(0,0,0,0.2)] xxl:w-[470px] xxl:p-5 xxl:rounded-xl"
            key={index}
          >
            ...
            <div className="p-2 w-full relative flex-col flex-center">
              <h4 className="bold-text mt-4 leading-[1.5] self-start">
                {work.title}
              </h4>
